					...
					if($respuesta["verificacion"] ==0){
						echo '<script>
							swal({
								type:"error",
								title: "¡ERROR!",
								text: "¡El correo electrónico aún no ha sido verificado, por favor revise la bandeja de entrada o la caroeta de SPAM de su correo electrónico para verificar la cuenta, o contactese con nuestro soporte a info@academyoflife.com !",
								showConfirmButton: true,
								confirmButtonText: "Cerrar"
							}).then(function(result){
								if(result.value){
		...
		if (isset($_POST["idUsuarioFoto"])) {
			...
			if (isset($_FILES["cambiarImagen"]["tmp_name"]) && !empty($_FILES["cambiarImagen"]["tmp_name"])) {
				...
				if ($_FILES["cambiarImagen"]["type"] == "image/jpeg") {
					...
					imagecopyresized($destino, $origen, 0, 0, 0, 0, $nuevoAncho, 
						$nuevoAlto, $ancho, $alto);
		...
		if (isset($_POST["emailRecuperarPassword"])) {
			if(preg_match('/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $_POST["emailRecuperarPassword"])){
				...
				if($traerUsuario){
					...
					$item = "password";
					$valor = $encriptar;
					$actualizarPassword = ModeloUsuarios:: mdlActualizarUsuario(
						$tabla,$id,$item, $valor);
					...
					echo '<script>
						swal({
							type:"error",
							title: "ERROR!",
							text: "¡El correo no existe en el sistema, verifique!",
							showConfirmButton: true,
							confirmButtonText: "Cerrar"					  
						}).then(function(result){
							if(result.value){   
